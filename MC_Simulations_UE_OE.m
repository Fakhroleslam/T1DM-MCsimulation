...
switch SimCase
    case 'NE', MealValueUncertainty = [-10 10]; % Normal eating [%]
    case 'UE', MealValueUncertainty = [-30 0]; % Undereating [%]
    case 'OE', MealValueUncertainty = [0 30]; % Overeating [%]
end
...
for cc = 1:N_sim % counter
    ...
    x_F = x_F/x_sum;
    x_P = x_P/x_sum;
    MVU = MealValueUncertainty/100;
    EI  = 1+MVU(1)+(MVU(2)-MVU(1))*rand; % Eating Index {under-, normal-, and over-eating}
...
switch SimCase
    case 'NE', save Results_NE.mat % Normal eating
    case 'UE', save Results_UE.mat % Undereating
    case 'OE', save Results_OE.mat % Overeating
end
